...
public class RockPaperScissorsGame {
    ...
    public static void main(String[] args) {
        ...
        String[] computer={"Scissors","Paper","Rock","Paper","Scissors"};
        int wins=0,losses=0,draws=0;
        System.out.println("Round | Player | Computer | Result");
        for(int i=0;i<player.length;i++){String r=playRound(player[i],computer[i]);System.out.printf("%d | %s | %s | %s%n",i+1,player[i],computer[i],r);if(r.startsWith("Player"))wins++;else if(r.startsWith("Computer"))losses++;else draws++;}
