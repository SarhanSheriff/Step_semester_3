...
public class FeeAccountSystem {
    static class FeeAccount {
        ...
        void pay(double amount) {
            if(amount>0)amountPaid=Math.min(totalFee,amountPaid+amount);
        }
        double getDue() {
    ...
    static class HostelFeeAccount extends FeeAccount {
        HostelFeeAccount(String r,double f) {
            super(r,f);
        }
    ...
    public static void main(String[]a) {
        ...
        for(FeeAccount x:new FeeAccount[] {
            p,h,s
        }
        ) {
            if(x instanceof HostelFeeAccount)System.out.println("Hostel account due: Rs "+x.getDue());
            else if(x instanceof ScholarshipFeeAccount)System.out.println("Scholarship account effective due: Rs "+((ScholarshipFeeAccount)x).effectiveDue());
            else System.out.println("Plain account due: Rs "+x.getDue());
        }
    }
}
