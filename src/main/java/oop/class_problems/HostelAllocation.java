...
public class HostelAllocation {
    ...
    static HostelRoom findAvailableRoom(HostelRoom[] rooms) {
        for(HostelRoom r:rooms)if(r!=null&&r.occupied<r.beds)return r;
        return null;
    }
    static void safeAllot(HostelRoom[] rooms,String studentName) {
        HostelRoom r=findAvailableRoom(rooms);
        if(r==null)System.out.println("No rooms available for "+studentName);
        else r.allot(studentName);
    }
    public static void main(String[]a) {
        HostelRoom[] r= {
            new HostelRoom("C-214",3,2),new HostelRoom("C-507",2,2)
        }
        ;
        safeAllot(r,"Divya");
        safeAllot(r,"Divya");
